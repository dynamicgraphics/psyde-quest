...
async function startQuest() {
  ...
  try {
    ...
    const validDomains = ['https://dynamicgraphics.github.io/psyde-quest/start', 'http://192.168.1.100/start'];
...
async function updateProgress() {
  ...
  const progress = userData.beaconsFound.length * 20; // 20% per beacon
...
async function triggerMechanic(type) {
  if (Date.now() - lastMechanicAttempt < 3000) return alert('Please wait 3 seconds between attempts');
...
async function completeBeacon() {
  ...
  const isAdvanced = ['beacon_03', 'beacon_04'].includes(currentBeacon.id); // Example advanced areas
  userData.score += isAdvanced ? 200 : 100; // Bonus for advanced areas
...
async function scanBeaconQR() {
  ...
  const qrData = await scanQR();
  const validDomains = ['https://dynamicgraphics.github.io/psyde-quest/admin', 'http://192.168.1.100/admin'];
  if (validDomains.some(domain => qrData.startsWith(domain))) {
    const beaconId = new URL(qrData).searchParams.get('beacon');
...
async function verifyPrize() {
  if (!isAdmin) return alert('Admin access required');
  const qrData = await scanQR();
  const validDomains = ['https://dynamicgraphics.github.io/psyde-quest/prize', 'http://192.168.1.100/prize'];
  if (validDomains.some(domain => qrData.startsWith(domain))) {
